...
'    datefrom = DateAdd("n", 59, datefrom)
    If Session.IsORACLE Then
        Filter = Filter & "(ChangeStamp >= " & MakeORACLEDate(datefrom) & ")"
    End If
    If Session.IsMSSQL Then
        Filter = Filter & "(ChangeStamp >= " & MakeMSSQLDate(datefrom) & ")"
    End If
    If Session.IsPOSTGRESQL Then
        Filter = Filter & "(ChangeStamp >= " & MakePGSQLDate(datefrom) & ")"
    End If
  End If
  If (Not IsNull(DTPicker2.Value)) Then
    ...
    End If
    If Session.IsPOSTGRESQL Then
        Filter = Filter & "(ChangeStamp < " & MakePGSQLDate(dateto) & ")"
    End If
  End If
  Dim ij As Long
 ...
 If Not item Is Nothing Then
    If (Filter <> vbNullString) Then
      Filter = Filter & " AND "
    End If
     Filter = Filter & " LogInstanceid='" & item.ID & "'"
 End If
...
'  the_comment
  ...
  Else
    Set rs = Session.GetRowsEx("V_Log_view", , , "ChangeStamp >=" & MakeMSSQLDate(LastCheckDate) & " and ChangeStamp < " & MakeMSSQLDate(NextCheckDate) & " ", " order by ChangeStamp")
  End If
  If Not rs Is Nothing Then
    While Not rs.EOF
...
Public Sub RefreshOrAppendRow(ByVal RowID As String)
  ...
  Set rs = Session.GetRows("v_LOG_view", , , "syslogID='" & RowID & "'")
  ...
  If RowNum = -1 Then
      ...
      mvarFieldArray(-0, cnt + 1) = "V_LOG_view"
      cnt = cnt + 1
      RowNum = cnt
  End If
...
Private Sub Form_Load()
  ...
  RefreshLog
  If Not item Is Nothing Then
    Me.Caption = "Протокол работы с документом: " & item.Name
  End If
End Sub
