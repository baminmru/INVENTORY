   ...
   Begin VB.Frame Frame 
      ...
      Begin VB.Frame frameRight 
         ...
         Begin VB.Label lblLogin 
            Caption         =   "имя пользователя PGSQL:"
            Height          =   255
            Index           =   1
            Left            =   180
            TabIndex        =   17
         ...
         Begin VB.Label lblPassword 
            Caption         =   "пароль пользователя PGSQL:"
            Height          =   255
            Index           =   1
            Left            =   180
            TabIndex        =   20
      ...
      Begin VB.Label Label4 
         Alignment       =   2  'Center
         Caption         =   "На этой вкладке задаются параметры НОВОЙ базы данных."
         ForeColor       =   &H00FF0000&
         Height          =   495
         Left            =   240
         TabIndex        =   27
...
Private Sub cmdTest_Click(Index As Integer)
  ...
  If Not ds.ServerLogIn Then
    If Index = 1 Then
      ...
      MsgBox "Не удается подключиться к PostgerSQL", vbCritical, "Ошибка"
    End If
    Set ds = Nothing
    Exit Sub
  Else
...
Private Sub Install()
    If MsgBox("Начать создание новой базы данных?", vbQuestion + vbYesNo) = vbYes Then
        ...
        lblPass = "Создание базы данных"
        MakeNewDB
        lblPass = "Доп. модификации базы данных"
        MakeUpdate
        lblPass = "Инициализация базы данных"
        ...
        ds.Integrated = False
        If Not ds.ServerLogIn Then
          MsgBox "Не удается подключиться к PotgreSQL Server", vbCritical
          Set ds = Nothing
          Exit Sub
        End If
        Set rs = ds.OpenRecordset("select initbase()")
...
Private Sub MakeNewDB()
  ...
  Dim sss   As String
    sss = ""
    sss = sss & vbCrLf & "SET search_path = public;"
    ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_nil()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_nil'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_ns_dns()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_ns_dns'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_ns_url()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_ns_url'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_ns_oid()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_ns_oid'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_ns_x500()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_ns_x500'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_generate_v1()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_generate_v1'"
    sss = sss & vbCrLf & "VOLATILE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_generate_v1mc()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_generate_v1mc'"
    sss = sss & vbCrLf & "VOLATILE STRICT LANGUAGE C;"
      ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_generate_v3(namespace uuid, name text)"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_generate_v3'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
        ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_generate_v4()"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_generate_v4'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
       ds.Execute sss
    sss = ""
    sss = sss & vbCrLf & "CREATE OR REPLACE FUNCTION uuid_generate_v5(namespace uuid, name text)"
    sss = sss & vbCrLf & "RETURNS uuid"
    sss = sss & vbCrLf & "AS '$libdir/uuid-ossp', 'uuid_generate_v5'"
    sss = sss & vbCrLf & "IMMUTABLE STRICT LANGUAGE C;"
...
'  If Not ds.Execute("use [" & txtDatabase(1).Text & "]") Then
'    MsgBox "Не удается обновить базу данных", vbCritical
'    Set ds = Nothing
'    Set GenResp = Nothing
'    Set GenPrj = Nothing
'    Exit Sub
